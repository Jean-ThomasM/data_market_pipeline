...
with st.sidebar:
    ...
    etat_choix = st.radio(
        "État de l'entreprise", ["Tous", "Actifs uniquement", "Fermés uniquement"]
    )
...
if lancer:
    if not api_key:
        ...
        st.warning(
            "⚠️ Veuillez remplir au moins un critère de recherche pour ne pas surcharger l'API."
        )
    else:
        with st.spinner("Interrogation de l'API Sirene en cours..."):
            ...
            if nom:
                ...
                q_parts.append(
                    f'(denominationUniteLegale:"{nom_clean}" OR nomUniteLegale:"{nom_clean}")'
                )
            if naf:
            ...
            params = {"q": q_query, "nombre": nb_max}
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if not etablissements:
                        st.info(
                            "ℹ️ Aucun établissement ne correspond à ces critères exacts."
                        )
                    else:
                            ...
                            lignes.append(
                                {
                                    ...
                                    "enseigne": e.get("periodesEtablissement", [{}])[
                                        0
                                    ...
                                    "effectifs": e.get(
                                        "trancheEffectifsEtablissement", "Non renseigné"
                                    ),
                                }
                            )
                    ...
                    st.error(
                        "⛔ Erreur 403 : Votre clé API Insee est invalide ou expirée."
                    )
                elif response.status_code == 400:
                    st.error(
                        f"❌ Erreur 400 : Syntaxe de requête invalide. (Requête envoyée : {q_query})"
                    )
                else:
                    st.error(
                        f"⚠️ Erreur Inconnue {response.status_code} : {response.text}"
                    )
