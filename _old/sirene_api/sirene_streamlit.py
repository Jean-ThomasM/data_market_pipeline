...
if search_button:
    if not SIRENE_API_KEY:
        ...
        query_list = []
        if nom_entreprise:
            query_list.append(f'denominationUniteLegale:"{nom_entreprise}"')
        if ville:
            query_list.append(f"libelleCommuneEtablissement:{ville}")
        if code_naf:
            query_list.append(f"activitePrincipaleUniteLegale:{code_naf}")
                            ...
                            df_data.append(
                                {
                                    ...
                                    "Nom": str(
                                        e.get("uniteLegale", {}).get(
                                            "denominationUniteLegale", ""
                                        )
                                    ),
                                    "Ville": str(
                                        e.get("adresseEtablissement", {}).get(
