...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Split a source parquet file into smaller parquet files in "
...
def split_parquet(
    input_path: Path, output_dir: Path, rows_per_file: int, prefix: str
) -> tuple[int, int]:
    try:
    ...
    for file_count, batch in enumerate(
        ...
        output_path = output_dir / f"{prefix}_{file_count:06d}.parquet"
